{
  ...
  home.file.".zeroclaw/workspace/skills/sentry".source = ../skills/sentry;
  ...
  home.activation.zeroclawSyncConfig = lib.hm.dag.entryAfter ["writeBoundary"] ''
    ...
    if [ -f "$sentry_token_file" ]; then
      sentry_token="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$sentry_token_file")"
    fi
    ...
    if [ -n "$sentry_token" ]; then
      tmp_file="$(mktemp)"
      {
        ...
        if [ -f "$sentry_base_url_file" ]; then
          sentry_base_url="$(${pkgs.coreutils}/bin/tr -d '\r\n' < "$sentry_base_url_file")"
      ...
      ${pkgs.coreutils}/bin/mv "$tmp_file" "$sentry_env_file"
